# shimataro/ssh-key-action

Install SSH key in ~/.ssh

Hardened by [Chainguard](https://www.chainguard.dev) from the upstream action at [https://github.com/shimataro/ssh-key-action](https://github.com/shimataro/ssh-key-action).

## Versions

| Version | Tag | Upstream commit |
|---------|-----|-----------------|
| v2.6.0 | [`v2.6.0`](https://github.com/chainguard-actions/shimataro-ssh-key-action/tree/v2.6.0) | [`d05e1bd`](https://github.com/shimataro/ssh-key-action/commit/d05e1bdf7d68cd4c94d0ce4bde253b89fa5bee23) |
| v2.6.1 | [`v2.6.1`](https://github.com/chainguard-actions/shimataro-ssh-key-action/tree/v2.6.1) | [`38b53cb`](https://github.com/shimataro/ssh-key-action/commit/38b53cb2f445ea2e0eb8872407e366677c41dbc6) |
| v2.7.0 | [`v2.7.0`](https://github.com/chainguard-actions/shimataro-ssh-key-action/tree/v2.7.0) | [`d4fffb5`](https://github.com/shimataro/ssh-key-action/commit/d4fffb50872869abe2d9a9098a6d9c5aa7d16be4) |
| v2.8.0 | [`v2.8.0`](https://github.com/chainguard-actions/shimataro-ssh-key-action/tree/v2.8.0) | [`6b84f2e`](https://github.com/shimataro/ssh-key-action/commit/6b84f2e793b32fa0b03a379cadadec75cc539391) |
| v2.8.1 | [`v2.8.1`](https://github.com/chainguard-actions/shimataro-ssh-key-action/tree/v2.8.1) | [`87a8f06`](https://github.com/shimataro/ssh-key-action/commit/87a8f067114a8ce263df83e9ed5c849953548bc3) |

## Privacy

This Action contacts Chainguard's licensing server to verify authorization. Connection metadata (IP address, GitHub repository identifier, timestamp, and any metadata encoded in the auth token) is transmitted to Chainguard, Inc. even if authorization is denied in accordance with our [Privacy Notice](https://www.chainguard.dev/legal/privacy-notice)
